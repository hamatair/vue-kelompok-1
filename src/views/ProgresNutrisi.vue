...
const rawUser = computed(() => authStore.user ?? {})
console.log('RAW USER DATA:', rawUser.value)

const userData = computed(() => {
  return {
    weight: Number(rawUser.value.weight_kg) || 60,
    height: Number(rawUser.value.height_cm) || 165,
    birthDate: rawUser.value.date_of_birth || '2000-01-01',
    gender: rawUser.value.gender || 'male',
    activityLevel: Number(rawUser.value.activity) || 1.2,
  }
})
...
const bmiStatus = computed(() => {
  ...
  if (val < 18.5)
    return {
      ...
      risk: 'Rendah (Risiko Defisiensi)',
      text: 'text-blue-500',
      bg: 'bg-blue-500',
      border: 'border-blue-500',
      desc: 'Tingkatkan asupan kalori padat gizi dan protein.',
    }
  if (val >= 18.5 && val < 24.9)
    return {
      label: 'Normal',
      risk: 'Rata-rata (Sehat)',
      text: 'text-green-500',
      bg: 'bg-green-500',
      border: 'border-green-500',
      desc: 'Pertahankan pola makan dan aktivitas fisik saat ini.',
    }
  if (val >= 25 && val < 29.9)
    return {
      ...
      desc: 'Disarankan defisit kalori ringan dan olahraga rutin.',
    }
  return {
    label: 'Obesitas',
    risk: 'Tinggi',
    ...
    desc: 'Konsultasikan diet ketat dan hindari gula/lemak jenuh.',
  }
})
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
      <div class="xl:col-span-8 flex flex-col gap-8">
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div
            class="lg:col-span-3 bg-white rounded-[2rem] p-0 relative min-h-[600px] overflow-hidden shadow-sm"
          >
            <div class="absolute top-8 left-8 z-20 dropdown-container">
              <button
                @click.stop="toggleDropdown('body')"
                class="bg-[#FFC700] text-xs font-bold px-4 py-2 rounded-full flex items-center gap-2 hover:bg-yellow-400 transition shadow-sm"
              >
                {{ selected.body }}
                <svg
                  ...
                  :class="activeDropdown === 'body' ? 'rotate-180' : ''"
                >
                  <path
              ...
              <div
                v-if="activeDropdown === 'body'"
                class="absolute top-10 left-0 mt-2 w-40 bg-white rounded-xl shadow-xl py-2 z-50 border border-gray-100 animate-fade-in"
              >
                <div
                  v-for="opt in dropdownData.body"
                  :key="opt"
                  @click="selectOption('body', opt)"
            ...
            <div class="w-full h-full flex justify-center items-center pt-10">
              <img
                src="/body-model.png"
                alt="Body Model"
                class="h-[500px] object-contain opacity-80 z-0"
              />
            </div>
            <div class="absolute top-[30%] right-[5%] flex items-center w-[45%]">
              <div class="flex-grow h-[1px] bg-orange-400 relative">
                <div
                  class="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 bg-orange-500 rounded-full border-2 border-white"
                ></div>
              </div>
              <div class="ml-2 text-right min-w-[80px]">
                <p class="text-xs text-gray-400">Dada</p>
                <p class="font-bold text-lg">93.0 <span class="text-xs">cm</span></p>
              </div>
            </div>
            <div class="absolute top-[40%] right-[10%] flex items-center w-[40%]">
              ...
              <div class="ml-2 text-right min-w-[80px]">
                ...
                <p class="font-bold text-lg">77.5 <span class="text-xs">cm</span></p>
              </div>
            </div>
            <div class="absolute top-[55%] right-[12%] flex items-center w-[35%]">
              ...
              <div class="ml-2 text-right min-w-[80px]">
                ...
                <p class="font-bold text-lg">58.5 <span class="text-xs">cm</span></p>
              </div>
            </div>
            <div class="absolute top-[34%] left-[8%] flex items-center flex-row-reverse w-[35%]">
              <div class="flex-grow h-[1px] bg-orange-400 relative">
                <div
                  class="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-3 h-3 bg-orange-500 rounded-full border-2 border-white"
                ></div>
              </div>
              <div class="mr-2 text-left min-w-[80px]">
                <p class="text-xs text-gray-400">Lengan</p>
                <p class="font-bold text-lg">28.5 <span class="text-xs">cm</span></p>
              </div>
            </div>
            <div class="absolute top-[45%] left-[10%] flex items-center flex-row-reverse w-[40%]">
              ...
              <div class="mr-2 text-left min-w-[80px]">
                ...
                <p class="font-bold text-lg">98.0 <span class="text-xs">cm</span></p>
              </div>
            </div>
          </div>
          ...
          <div class="lg:col-span-2 flex flex-col gap-8">
            <div class="bg-[#FFF8ED] rounded-[2rem] p-8 shadow-sm relative">
              ...
              <div class="space-y-2 mb-4">
                <div class="flex justify-between text-sm">
                  ...
                  ><span class="font-bold text-xl">{{ userData.weight }} Kg</span>
                </div>
                <div class="flex justify-between text-sm">
                  <span class="text-gray-500">Tinggi</span
                  ><span class="font-bold text-xl">{{ userData.height }} cm</span>
                </div>
              </div>
              <div class="h-32 w-full relative">
                <svg class="w-full h-full" viewBox="0 0 300 100" preserveAspectRatio="none">
                  <path
                    d="M0,80 C80,90 150,70 220,60 L300,55 L300,100 L0,100 Z"
                    fill="#FDE68A"
                    opacity="0.4"
                  />
                  <path
                    d="M0,80 C80,90 150,70 220,60 L300,55"
                    fill="none"
                    stroke="#F59E0B"
                    stroke-width="3"
                    stroke-linecap="round"
                ...
                <div
                  class="absolute top-[40%] left-[60%] bg-white text-[10px] font-bold px-2 py-1 rounded shadow text-orange-500 border border-orange-100"
                >
                  {{ userData.weight }} kg
                </div>
              </div>
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-sm">
              <div class="flex justify-between items-center mb-4">
                ...
                <button class="bg-[#FFC700] text-[10px] px-3 py-1 rounded-full font-bold">
                  View All
                </button>
              </div>
              <div class="flex gap-3">
                <div class="flex-1">
                  ...
                  <div class="bg-gray-200 h-32 rounded-xl relative overflow-hidden">
                    <div
                      class="absolute bottom-1 left-1 bg-white/80 px-2 py-0.5 rounded text-[10px] font-bold"
                    >
                      82 Kg
                    </div>
        ...
        <div class="bg-[#FFF8ED] rounded-[2rem] p-8 shadow-sm">
          <div class="mb-4">
            <button class="bg-white px-4 py-2 rounded-xl text-sm font-bold shadow-sm text-gray-600">
              September 2025 v
            </button>
          </div>
      ...
      <div class="xl:col-span-4 flex flex-col gap-8">
        <div class="bg-white rounded-[2rem] p-8 shadow-sm relative z-30">
          <div class="flex justify-between mb-2 items-center">
            ...
            <div class="relative dropdown-container">
              <button
                @click.stop="toggleDropdown('calorie')"
                class="bg-[#FFC700] text-[10px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1 hover:bg-yellow-400 transition shadow-sm"
              >
                {{ selected.calorie }}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-2.5 h-2.5 transition-transform"
                  :class="activeDropdown === 'calorie' ? 'rotate-180' : ''"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="3"
              ...
              <div
                v-if="activeDropdown === 'calorie'"
                class="absolute top-8 right-0 w-36 bg-white rounded-lg shadow-xl py-2 z-50 border border-gray-100 animate-fade-in"
              >
                <div
                  v-for="opt in dropdownData.calorie"
                  :key="opt"
                  @click="selectOption('calorie', opt)"
                  class="px-3 py-1.5 text-[10px] hover:bg-gray-50 cursor-pointer"
                >
                  {{ opt }}
                </div>
              </div>
          ...
          <div class="mb-8 relative">
            <h2 class="text-3xl font-extrabold text-gray-800">
              {{ dailyTarget }}
              <span class="text-sm font-normal text-gray-400 ml-1">kkal/hari</span>
            </h2>
          ...
          <div class="flex flex-grow gap-3 h-48">
            <div
              class="flex flex-col justify-between h-full text-[10px] text-gray-300 pb-6 text-right w-8 font-medium"
            >
            ...
            <div
              class="flex justify-between items-end w-full pb-6 h-full relative border-l border-dashed border-gray-200 pl-4"
            >
              <div
                class="absolute top-[35%] w-full border-t border-dashed border-green-200 z-0"
              ></div>

              <div
                v-for="d in calorieData"
                :key="d.day"
                class="flex flex-col justify-end items-center h-full w-1/4 relative group z-10"
              >
                <div
                  v-if="d.isTarget"
                  class="absolute top-[15%] bg-orange-100 border border-orange-200 shadow-sm text-[10px] font-bold px-2 py-1 rounded mb-1 z-20 whitespace-nowrap text-orange-600"
                >
                  {{ d.label }}
                </div>
                <div class="flex gap-1.5 items-end h-full w-full justify-center">
                  <div
                    class="w-3 bg-[#FFC700] rounded-t-lg transition-all hover:bg-yellow-300 shadow-sm"
                    :style="{ height: (d.intake / (dailyTarget + 500)) * 100 + '%' }"
                  ></div>
                  <div
                    class="w-3 bg-orange-400 rounded-t-lg transition-all hover:bg-orange-300 shadow-sm"
                    :style="{ height: (d.burn / (dailyTarget + 500)) * 100 + '%' }"
                  ></div>
                </div>
          ...
          <div class="flex justify-center gap-6 text-[10px] text-gray-500 font-medium mt-2">
            <div class="flex items-center gap-2">
              <div class="w-2.5 h-2.5 rounded-full bg-[#FFC700]"></div>
              Masuk
            </div>
            <div class="flex items-center gap-2">
        ...
        <div class="bg-white rounded-[2rem] p-8 shadow-sm relative z-20 flex flex-col items-center">
          <h3 class="font-bold text-gray-800 self-start mb-6 w-full flex justify-between">
            Indeks Massa Tubuh
            <span class="text-xs font-normal text-gray-400 bg-gray-50 px-2 py-1 rounded-lg"
              >BMI</span
            >
          </h3>

          <div
            class="relative w-44 h-44 flex flex-col items-center justify-center rounded-full border-[10px] transition-colors duration-500 bg-gray-50"
            :class="bmiStatus.border"
          >
            <span class="text-5xl font-black text-gray-800 tracking-tight">{{ bmi }}</span>
            <span class="text-xs uppercase tracking-widest font-bold mt-1 text-gray-400"
              >Score</span
            >
          </div>

          <div
            class="mt-6 px-6 py-2 rounded-full text-sm font-bold shadow-md text-white transition-colors duration-500"
            :class="bmiStatus.bg"
          >
            {{ bmiStatus.label }}
          </div>
          ...
          <p
            class="text-center text-xs text-gray-500 mt-6 px-4 leading-relaxed bg-gray-50 p-3 rounded-xl w-full border border-gray-100"
          >
            Berdasarkan tinggi <strong>{{ userData.height }}cm</strong> dan berat
        ...
        <div class="bg-white rounded-[2rem] p-8 shadow-sm relative z-10">
          ...
          <div
            class="flex items-center gap-4 mb-8 bg-green-50 p-5 rounded-2xl border border-green-100"
          >
          ...
          <div class="space-y-3">
            <div class="flex justify-between text-xs mb-1 font-medium">
              ...
              <span class="font-bold transition-colors duration-300" :class="bmiStatus.text">{{
                bmiStatus.risk
              }}</span>
            </div>
            <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden shadow-inner">
              <div
                ...
                :class="bmiStatus.bg"
                :style="{
                  width: bmi >= 30 ? '100%' : bmi >= 25 ? '75%' : bmi >= 18.5 ? '25%' : '10%',
                }"
              ></div>
            </div>

            <div
              class="mt-4 p-4 bg-gray-50 rounded-2xl text-[11px] text-gray-500 leading-relaxed border border-gray-100"
            >
