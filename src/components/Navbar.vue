  ...
  <nav class="w-full px-6 py-4 flex justify-between items-center bg-orange text-white-soft">
    ...
    <div class="relative" ref="menuRef">
      ...
      <div v-if="showMenu"
        ...
        <div @click="closeMenu" class="hover:bg-orange-light/20 p-2 rounded cursor-pointer text-red-600">
          Logout
        </div>
      </div>
    </div>
  ...
  <div v-if="showFeedback" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[9999]"
    ...
    <div class="bg-white-soft p-8 rounded-xl shadow-xl w-[450px] max-w-full">
      ...
      <div class="mb-6">
        ...
        <div class="flex gap-2">
          <button v-for="n in 5" :key="n" @click="setRating(n)" class="text-3xl">
            <FontAwesomeIcon :icon="rating >= n ? 'fa-solid fa-star' : 'fa-regular fa-star'"
      ...
      <div>
        ...
        <textarea v-model="text" placeholder="Tambahkan feedback..." rows="4"
          class="w-full border border-orange-light rounded-md px-3 py-2"></textarea>
      </div>
